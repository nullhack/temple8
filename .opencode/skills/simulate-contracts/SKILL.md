...
8. Run the dev ruff check (`ruff check .`) on the whole project; the gate is zero violations. Restructure lint (`SIM`, `RUF`) and `ruff format` are merge-time per [[software-craft/docstring-lifecycle]] — a readability restructure is not a plan defect. Plan-authored tests and stubs must pass the bug-catcher set before build.
