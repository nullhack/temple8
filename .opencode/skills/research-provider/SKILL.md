...
4. Determine the access shape: endpoint base/URI, protocol, credentials source (a named variable the user must supply in `~/.secrets/<project>.env`), request/response shape, and both success and failure modes. Per [[software-craft/secrets-and-config]] the agent names the variable and instructs the user where to set it; it never holds or prints the value.
