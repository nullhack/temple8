...
3. `cd` into the new project and verify the skeleton: `<package>/__init__.py` exists; `tests/{integration,e2e,cassettes,fixtures}/` exist; `pyproject.toml` and `README.md` are rendered (no `{{ }}` placeholders remain); `.opencode/`, `.flowr/`, `.templates/`, `AGENTS.md`, `conftest.py`, `.github/` are present and unchanged.
