...
**Refactor under green, design only.** Refactor improves the `.py`'s structure while every test stays green, because the tests are the safety net that makes restructuring safe. Two boundaries are fixed: the `.pyi` does not move (a refactor that needs to change it has found a contract gap, escalated at review), and conventions do not run (no docstrings, no formatting pass — those are CI's job). Refactor is where SOLID, Object Calisthenics, and the smell catalogue are applied, each when its symptom appears, never speculatively.
...
| design improvement only | convention compliance (docstrings, formatting — CI's job) |
